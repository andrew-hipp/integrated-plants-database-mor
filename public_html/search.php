...
<?php
	...
	if ($_REQUEST['collectionCombo'] != '') {
		if ($needLeadingComma)
			echo ', ';
		// echo ' ------  ' . $_REQUEST['collectionCombo'] . ' ' . $collectionSite . ' ------ ';
		$collectionCombo = $_REQUEST['collectionCombo'];
...

{
	...
	if (mysqli_connect_error() == 0) {
		...
		if ($_REQUEST['collectionSite'] != '') {
			...
			if(!isset($_REQUEST['collectionCombo']))
			{
		...
		if($_REQUEST['collectionCombo'] == '') // If there is a collection combo, then do not do the herbarium results, otherwise do.
		{
